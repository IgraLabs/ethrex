...
generate_network_params_hash() {
    ...
    replace_network_params_vars "$template" "$output" || {
        fatal_error "Failed to generate network params" 5
    }
    ...
    echo "  EXTRA_DATA_SHORT: ${EXTRA_DATA_SHORT}"
    echo ""
}
...
generate_genesis_json() {
    ...
    replace_env_vars "$template" "$temp_file" || {
        fatal_error "Failed to generate genesis.json" 5
    }
...
print_genesis_hash() {
    echo "Computing genesis block hash..."
    ethrex compute-genesis-hash --path "$genesis_json"
}
...
display_ethrex_params() {
    ...
    echo "  HTTP: 0.0.0.0:${HTTP_PORT:-8545}"
    echo "  WS: 0.0.0.0:${WS_PORT:-8546}"
    echo "  Auth RPC: 0.0.0.0:${AUTHRPC_PORT:-8551}"
    echo "  Data Directory: ${data_dir}"
    echo "  Builder Extra Data: '${EXTRA_DATA_SHORT}' (full: ${EXTRA_DATA})"
...
start_ethrex_node() {
    ...
    exec ethrex \
        ...
        --metrics.port "${METRICS_PORT:-9001}" \
        --log.level "${ETHREX_LOG_LEVEL:-info}" \
        ${ETHREX_EXTRA_ARGS:-}
}
...
if [ "${IGRA_PRINT_GENESIS_HASH_AND_EXIT:-false}" = "true" ]; then
    print_genesis_hash
    exit 0
fi
